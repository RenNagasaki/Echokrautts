...
download_models() {
    ...
    # (~5 GB), XTTS one multilingual model (~2 GB), Chatterbox one multilingual
    # model (~2 GB). All are idempotent — the HF cache and Coqui's ModelManager
    ...
    case "$BACKEND" in
        ...
        chatterbox) log "ensuring Chatterbox weights in ${F5W_MODELS_DIR:-models} …"
                    python -m src.chatterbox_backend ;;
